...
class Integral(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        shape = x.size()  # N*4(reg_max+1)
        x = F.softmax(x.reshape(*shape[:-1], 4, self.reg_max + 1), dim=-1)  # N*4*(reg_max+1)
        x = F.linear(x, self.project.type_as(x)).reshape(*shape[:-1], 4)  # N*4
        return x
...
class GFLHead(nn.Module):
    ...
    def loss(self, preds, gt_meta):
        # preds bs*2100*(1+4*(reg_max+1))
        cls_scores, bbox_preds = preds.split(
            [self.num_classes, 4 * (self.reg_max + 1)], dim=-1
        )
        # cls_scores bs*2100*1
        # bbox_preds bs*2100*(4*(reg_max+1))
        device = cls_scores.device
        gt_bboxes = gt_meta["gt_bbox"]
        gt_labels = gt_meta["gt_label"]
        input_height, input_width = gt_meta["img"][0].shape[2:]
        ...
        (
            cls_preds_list,  # [(bs,1600,1),(bs,400,1),(bs,100,1)]
            reg_preds_list,  # [(bs,1600,32),(bs,400,32),(bs,100,32)]==>32=4*(7+1)
            grid_cells_list,  # [(bs,1600,4),(bs,400,4),(bs,100,4)]
            labels_list,  # [(bs,1600),(bs,400),(bs,100)]
            label_weights_list,  # [(bs,1600),(bs,400),(bs,100)]
            bbox_targets_list,  # [(bs,1600,4),(bs,400,4),(bs,100,4)]
            bbox_weights_list,  # [(bs,1600,4),(bs,400,4),(bs,100,4)]
            num_total_pos,
            num_total_neg,
        ) = cls_reg_targets
    ...
    def loss_single(
        ...
        cls_score = cls_score.reshape(-1, self.cls_out_channels)  # (bs*1600,1), (bs*400,1), (bs*100,1)
        bbox_pred = bbox_pred.reshape(-1,
                                      4 * (self.reg_max + 1))  # (bs*1600,4*(7+1)), (bs*400,4*(7+1)), (bs*100,4*(7+1))
        bbox_targets = bbox_targets.reshape(-1, 4)
        labels = labels.reshape(-1)  # 2100
        label_weights = label_weights.reshape(-1)  # 2100
        ...
        if len(pos_inds) > 0:
            ...
            pred_corners = pos_bbox_pred.reshape(-1, self.reg_max + 1)  # (n*4)*(reg_max+1)
    ...
    def target_assign(
        ...
        """
        cls_preds: bs*2100*1
        reg_preds: bs*2100*(4*(reg_max+1))
        featmap_sizes: [40*40,20*20,10*10]
        gt_bboxes_list: bs*1*4 单目标
        gt_bboxes_ignore_list: None
        gt_labels_list: bs*1*1 单目标
        ...
        # 生成若干40*40（1600个），80*80（400个），160*160（100个）的Anchor
        # multi_level_grid_cells=[1600*4, 400*4, 100*4]
        ...
        # 不同特征预测图的锚点数num_level_cells[1600,400,100]
        ...
        # 获得不同层次预测特征图的结果
        mlvl_cls_preds = images_to_levels([c for c in cls_preds], num_level_cells)  # [1600*1,400*1,100*1]
    ...
    def target_assign_single_img(
        ...
        num_cells = grid_cells.shape[0]  # 2100
        bbox_targets = torch.zeros_like(grid_cells)  # 2100*4
        bbox_weights = torch.zeros_like(grid_cells)  # 2100*4
        labels = grid_cells.new_full((num_cells,), self.num_classes, dtype=torch.long)  # 默认都为背景 (2100,)
        label_weights = grid_cells.new_zeros(num_cells, dtype=torch.float)  # 2100
    ...
    def show_result(
            self, img, dets, class_names, score_thres=0.1, show=True, save_path=None
    ):
    ...
    def get_bboxes(self, cls_preds, reg_preds, img_metas):
        ...
        b = cls_preds.shape[0]
        input_height, input_width = img_metas["img"][0].shape[2:]
        ...
        for i in range(b):
            ...
            results = multiclass_nms(
                ...
                score,
                score_thr=0.05,
                nms_cfg=dict(type="nms", iou_threshold=0.6),  # 阈值0.6可以调整
                max_num=100,
            )
            # print(results)
    ...
    def get_single_level_center_point(
        ...
        """
        shape 40*40: 将[4~316]每个数重复40遍
        shape 20*20: [8~312]同上， 10*10: [16~304]同上
        """
        y, x = torch.meshgrid(y_range, x_range)
        if flatten:
            y = y.flatten()
            x = x.flatten()
        # x或y： 40*40==>1600, 20*20==>400, 10*10==>100
        return y, x
    ...
    def get_grid_cells(self, featmap_size, scale, stride, dtype, device):
        ...
        # 40*40==>y(40),x(40), 20*20==>y(20),x(20), 10*10==>y(10),x(10)
        y, x = self.get_single_level_center_point(
            featmap_size, stride, dtype, device, flatten=True
        )

        """
        map_size=40*40, stride=8, cell_size= 40==>grid_cells: 1600*4
            [-16., -16.,  24.,  24.],
            [ -8., -16.,  32.,  24.],
            [  0., -16.,  40.,  24.]
            ...
            [280., 296., 320., 336.],
            [288., 296., 328., 336.],
            [296., 296., 336., 336.]  ===>小目标检测框，都为40*40
        map_size=20*20, stride=16, cell_size= 80==>grid_cells: 400*4
            [-32., -32.,  48.,  48.]
            [-16., -32.,  64.,  48.],
            [  0., -32.,  80.,  48.],
            ...
            [240., 272., 320., 352.],
            [256., 272., 336., 352.],
            [272., 272., 352., 352.] ===>中等目标检测框，都为80*80
        map_size=10*10, stride=32, cell_size= 160==>grid_cells: 100*4
            [-64., -64.,  96.,  96.],
            [-32., -64., 128.,  96.],
            [  0., -64., 160.,  96.],
            ...
            [160., 224., 320., 384.],
            [192., 224., 352., 384.],
            [224., 224., 384., 384.] ===>大目标检测框，都为160*160
        """
        grid_cells = torch.stack(
            [
                x - 0.5 * cell_size,  # 左上角x
                y - 0.5 * cell_size,  # 左上角y
                x + 0.5 * cell_size,  # 右下角x
                y + 0.5 * cell_size,  # 右下角y
            ],
            dim=-1,
        )
        # 处理边界值（不处理也可以，因为是Anchor-free）
        return grid_cells
