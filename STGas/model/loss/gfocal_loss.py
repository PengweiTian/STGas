...
def distribution_focal_loss(pred, label):
    ...
    # pred N*(reg_max+1)
    # label N
    dis_left = label.long()  # 转为长整型张量，浮点值会被截断为整数
    dis_right = dis_left + 1  # +1包含原本的浮点值
    weight_left = dis_right.float() - label  # 得到原本label的浮点值与最大取整的距离
    weight_right = label - dis_left.float()  # 得到原本label的浮点值与最小取整的距离
    """
    label=3.2152  ==>  dis_left=3,dis_right=4
    weight_left=4-3.2152=0.7848
    weight_right=3.2152-3=0.2152
    
    (reg_max+1)=7时  ==> pred=[-4.6084, -4.5939, -4.5790, -4.5968, -4.6083, -4.6206, -4.5980]
    """
    loss = (
