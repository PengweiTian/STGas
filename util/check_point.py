...
def save_model(mode, model, path, epoch, optimizer=None):
    model_state_dict = (
        model.module.state_dict() if hasattr(model, "module") else model.state_dict()
    )
    ...
    # ckpt
    torch.save(data, os.path.join(path, f"model_{mode}.ckpt"))
    # pth
    torch.save(model_state_dict, os.path.join(path, f"model_{mode}.pth"))
