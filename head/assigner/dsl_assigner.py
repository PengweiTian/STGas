...
class DynamicSoftLabelAssigner(BaseAssigner):
    ...
    def assign(
        ...
        prior_center = priors[:, :2]  # 预测中心
        # 先验框中心到GT框左上角的距离和右下角的距离
        lt_ = prior_center[:, None] - gt_bboxes[:, :2]
        rb_ = gt_bboxes[:, 2:] - prior_center[:, None]

        deltas = torch.cat([lt_, rb_], dim=-1)
        is_in_gts = deltas.min(dim=-1).values > 0  # 中心是否在GT框内
    ...
    def dynamic_k_matching(self, cost, pairwise_ious, num_gt, valid_mask):
        ...
        Args:
            cost (Tensor): Cost matrix. num_preds*num_gt
            pairwise_ious (Tensor): Pairwise iou matrix. num_preds*num_gt
            num_gt (int): Number of gt. num_gt
            valid_mask (Tensor): Mask for valid bboxes.
        """

        matching_matrix = torch.zeros_like(cost)  # num_preds*1
        # select candidate topk ious for dynamic-k calculation
        candidate_topk = min(self.topk, pairwise_ious.size(0))  # 13*1
        topk_ious, _ = torch.topk(pairwise_ious, candidate_topk, dim=0)  # 13*1
        # calculate dynamic k for each gt
        # topk_ious.sum(0).int()==>[13]

        dynamic_ks = torch.clamp(topk_ious.sum(0).int(), min=1)  # 将所有小于1的元素设置为1
